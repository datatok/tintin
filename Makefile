...
ifneq ($(BINARY_VERSION),)
	LDFLAGS += -X github.com/datatok/tintin/internal/version.version=${BINARY_VERSION}
endif

# Clear the "unreleased" string in BuildMetadata
ifneq ($(GIT_TAG),)
	LDFLAGS += -X github.com/datatok/tintin/internal/version.metadata=
endif
LDFLAGS += -X github.com/datatok/tintin/internal/version.gitCommit=${GIT_COMMIT}
LDFLAGS += -X github.com/datatok/tintin/internal/version.gitTreeState=${GIT_DIRTY}
...
$(BINDIR)/$(BINNAME): $(SRC)
	GO111MODULE=on go build $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(BINDIR)/$(BINNAME) ./cmd/tintin
...
.PHONY: format
format: $(GOIMPORTS)
	GO111MODULE=on go list -f '{{.Dir}}' ./... | xargs $(GOIMPORTS) -w -local github.com/datatok/tintin
...
build-cross: $(GOX)
	GO111MODULE=on CGO_ENABLED=0 $(GOX) -parallel=3 -output="_dist/{{.OS}}-{{.Arch}}/$(BINNAME)" -osarch='$(TARGETS)' $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LDFLAGS)' ./cmd/tintin
...
dist:
	( \
		...
		$(DIST_DIRS) tar -zcf tintin-${VERSION}-{}.tar.gz {} \; && \
		$(DIST_DIRS) zip -r tintin-${VERSION}-{}.zip {} \; \
	)
...
.PHONY: build-docker
build-docker:
	docker build -t datatok/tintin .
	docker tag datatok/tintin ${CI_REGISTRY_IMAGE}:${VERSION}
